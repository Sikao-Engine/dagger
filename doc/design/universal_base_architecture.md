# 通用批量 Agent 编排底座（工作名：**DivDag**）架构设计
...
1. [DivDag 要解决的问题与五个内核](#1-divdag-要解决的问题与五个内核)
2. [领域模型](#2-领域模型)
3. [分层与包结构](#3-分层与包结构)
...
divdag/
├── packages/
│   ├── divdag_kernel/            # 领域无关内核（无 HTTP、无 DB 依赖，可单测）
│   │   ├── dag/
...
│   │
│   └── divdag_cli/               # 给 Agent 用的确定性 CLI 脚手架
...
**依赖方向严格单向**：`domains → server → divdag_kernel`，`divdag_kernel` 不 import 任何领域符号，
...
```python
# divdag_kernel/executors/catalog.py
@dataclass(frozen=True)
class ExecutorSpec:
...
除「调度前分片列表已定」外，DivDag 支持 `Scope.SHARD_DYNAMIC`：
...
`StateSeeder` 写入 `<workspace>/.divdag/cursor.json`：
...
class DomainPlugin(Protocol):
    ...
    def state_seeder(self) -> StateSeeder | None: ...       # 写入 .divdag/cursor.json 等
...
底座不内置任何领域命令，只提供 `divdag_cli` 脚手架，约定三条契约：
...
- 不做通用工作流引擎（Airflow/Temporal 替代品）。DivDag 的定位是
