"""DivDag configuration model.

Pydantic model for the `divdag.yaml` schema. Loads from a path and gives readable
...
class DivDagConfig(BaseModel):
    """Top-level DivDag configuration."""

    model_config = ConfigDict(extra="forbid")

    data_dir: Path = Field(default=Path(".divdag"), description="State root for all runs.")
    workspaces_dir: Path = Field(
        default=Path(".divdag/workspaces"), description="Where WorkspaceProvider roots go."
    )
...
def load_config_from_dict(data: dict[str, Any]) -> DivDagConfig:
    """Construct a DivDagConfig from a parsed mapping. Raises pydantic.ValidationError on bad input."""
    return DivDagConfig.model_validate(data)


__all__ = ["AgentBackendConfig", "DomainConfig", "DivDagConfig", "load_config_from_dict"]
