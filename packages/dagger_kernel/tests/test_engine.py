...
from dagger_kernel.engine import (
    EngineHooks,
    NodeFailed,
    run_graph,
)
from dagger_kernel.executors import ExecutorCatalog, ExecutorSpec
...
class TestEngineHappyPath:
    def test_runs_full_graph_to_completion(self, catalog, nodes, store, tmp_path: Path) -> None:
        tpl = _tiny_template()
        shards = [
            __import__("dagger_kernel").dag.instantiator.ShardPlan(
...
class TestEngineRetries:
    def test_retries_until_success(self, catalog, nodes, store, tmp_path: Path) -> None:
        tpl = _tiny_template()
        shards = [__import__("dagger_kernel").dag.instantiator.ShardPlan(shard_id="s0", index=0)]
        graph = instantiate(template=tpl, run_id="run_e2e", shards=shards)
...
from dagger_kernel.dag.instantiator import ShardPlan  # noqa: E402
from dagger_kernel.spi import SkillSpec  # noqa: E402
...
class TestParallelWritesRule:
    ...
    def test_all_fanin_differing_values_raise_conflict(self, nodes, store, tmp_path: Path) -> None:
        from dagger_kernel.dag import ContextConflictError
