"""SPI: the single entry point domains implement to plug into Dagger.
...
The `DomainRegistry` discovers plugins via `entry_points` (the `dagger.domains`
...
KERNEL_EXECUTORS: tuple[ExecutorSpec, ...] = (
    ExecutorSpec(
        ...
        scope="any",
        node_class="dagger_kernel.dag.nodes.ensure_workspace:EnsureWorkspaceNode",
    ),
)
...
def discover_entry_points() -> list[DomainPlugin]:
    """Discover DomainPlugin instances via the `dagger.domains` entry-point group.
    ...
    try:
        ...
        eps = entry_points()
        group = eps.select(group="dagger.domains")
        for ep in group:
            try:
                obj = ep.load()
