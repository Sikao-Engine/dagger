...
import pytest
from divdag_agent.backend import AgentEvent
from divdag_agent.backends.opencode_http import OpencodeEndpoint, OpencodeHttpBackend
from divdag_agent.opencode import SSEEvent
from divdag_agent.opencode.process_manager import ManagedProcess, ProcessStatus
...
class _FakeClient:
    ...
    async def create_session(self, title=None):
        from divdag_agent.opencode import Session
...
class TestEnsureReady:
    async def test_ensure_ready_returns_endpoint_with_unique_key(self, tmp_path: Path) -> None:
        with patch("divdag_agent.backends.opencode_http.AgentCLIAsyncClient", _FakeClient):
        ...
        assert ep.process_key.startswith("divdag-")
