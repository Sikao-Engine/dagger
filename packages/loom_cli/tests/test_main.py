"""K6: CLI acceptance — `loom run --domain novel_digest --backend mock` really runs.

Also pins: `loom domains` lists both domains, the tiny domain still runs
...
from loom_cli.main import _build_registry, _default_domains_dir, cli
...
class TestRunNovelDigest:
    def test_mock_run_end_to_end(self, tmp_path: Path, monkeypatch) -> None:
        ...
        monkeypatch.chdir(tmp_path)  # state lands under tmp_path/.loom
        ...
        runs = list((tmp_path / ".loom").glob("run_*"))
        assert len(runs) == 1
        state = runs[0] / "state"
