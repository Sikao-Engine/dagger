"""divdag CLI: run, state, config, domains.

`divdag run --domain <id> --items ./data --shards 3` runs a domain's default
template end-to-end with the mock backend (or a configured real backend).

Domain discovery: entry points (`divdag.domains` group) first; as a dev fallback,
...
from divdag_cli.scaffold import die
...
def _default_domains_dir() -> Path:
    ...
    # main.py lives at <repo>/packages/divdag_cli/src/divdag_cli/main.py → parents[4]
    # is the repo root.
    return Path(__file__).resolve().parents[4] / "domains"


def _load_config_or_default() -> dict[str, Any]:
    """Load divdag.yaml if present; else return a default config."""
    from divdag_kernel.config import load_config_from_dict

    path = Path.cwd() / "divdag.yaml"
    if path.exists():
        try:
            ...
            click.echo(
                "warning: ruyaml not installed; install the [yaml] extra for divdag.yaml parsing",
                err=True,
            )
            data = {}
...
@click.group()
def cli() -> None:
    """DivDag: batch Agent orchestration CLI."""
