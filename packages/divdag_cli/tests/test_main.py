"""K6: CLI acceptance — `divdag run --domain novel_digest --backend mock` really runs.

Also pins: `divdag domains` lists both domains, the tiny domain still runs
...
from divdag_cli.main import _build_registry, _default_domains_dir, cli
...
class TestRunNovelDigest:
    def test_mock_run_end_to_end(self, tmp_path: Path, monkeypatch) -> None:
        ...
        monkeypatch.chdir(tmp_path)  # state lands under tmp_path/.divdag
        ...
        runs = list((tmp_path / ".divdag").glob("run_*"))
        assert len(runs) == 1
        state = runs[0] / "state"
