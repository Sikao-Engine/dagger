...
KERNEL_SRC = Path(__file__).resolve().parents[2] / "src" / "loom_kernel"
...
PATH_BUILDING_ALLOWLIST = {
    "loom_kernel/state/layout.py",
    "loom_kernel/state/store.py",
    "loom_kernel/state/io.py",  # tmp-file naming + os.replace
}
...
class TestNoRawPathConcat:
    ...
    def test_state_path_construction_lives_only_in_layout(self) -> None:
        ...
        for src in _iter_py_files(KERNEL_SRC):
            rel = _rel_to_kernel(src)
            if rel == "loom_kernel/state/layout.py":
                continue
            text = src.read_text(encoding="utf-8")
