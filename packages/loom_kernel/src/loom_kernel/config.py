"""Loom configuration model.

Pydantic model for the `loom.yaml` schema. Loads from a path and gives readable
...
class LoomConfig(BaseModel):
    ...
    data_dir: Path = Field(default=Path(".loom"), description="State root for all runs.")
    workspaces_dir: Path = Field(
        default=Path(".loom/workspaces"), description="Where WorkspaceProvider roots go."
    )
...
def load_config_from_dict(data: dict[str, Any]) -> LoomConfig:
    """Construct a LoomConfig from a parsed mapping. Raises pydantic.ValidationError on bad input."""
    return LoomConfig.model_validate(data)


__all__ = ["AgentBackendConfig", "DomainConfig", "LoomConfig", "load_config_from_dict"]
