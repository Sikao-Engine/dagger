...
import pytest
from dagger_agent.backend import AgentEvent
from dagger_agent.backends.opencode_http import OpencodeEndpoint, OpencodeHttpBackend
from dagger_agent.opencode import SSEEvent
from dagger_agent.opencode.process_manager import ManagedProcess, ProcessStatus
...
class _FakeClient:
    ...
    async def create_session(self, title=None):
        from dagger_agent.opencode import Session
...
class TestEnsureReady:
    async def test_ensure_ready_returns_endpoint_with_unique_key(self, tmp_path: Path) -> None:
        with patch("dagger_agent.backends.opencode_http.AgentCLIAsyncClient", _FakeClient):
        ...
        assert ep.process_key.startswith("dagger-")
