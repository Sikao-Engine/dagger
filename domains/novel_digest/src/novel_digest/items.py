...
- The frontier (resume pointer) is read from `<root>/.divdag/cursor.json` when
...
from divdag_kernel.planning import ItemLedgerSnapshot, Milestone, WorkItem
...
def read_frontier(root: Path) -> str | None:
    """Read `.divdag/cursor.json`'s cursor_item_id if the file exists."""
    cursor = Path(root, ".divdag", "cursor.json")
