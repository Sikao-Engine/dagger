...
Second domain — validates that DivDag's abstractions generalize beyond `tiny`.
...
`divdag run --domain novel_digest --backend mock` 真实 CLI 跑通；为补齐内核缺口做了
...
一次性安装（与工作区解耦，详见 `doc/plan/novel_digest_acceptance.md` §0.5）：
...
uv run server/main_server.py -c my-divdag.toml          # 4. 按 config 打开工作区
```
...
uv run divdag run --domain novel_digest --items /tmp/book --shards 2 --backend mock
...
```
domains/novel_digest/
├── install.py     # 装 skills_md/ 到 ~/.agents/skills（stdlib only，--dest/--dry-run，幂等）
...
└── src/novel_digest/
    ...
    ├── items.py       # ChapterSource：扫 chapters/*.txt，卷边界 → Milestone，frontier 读 .divdag/cursor.json
    ...
    ├── noveltool/     # 确定性 CLI 实现：status / next / fill / check（JSON + 退出码 0/1/2/3 + 幂等）
