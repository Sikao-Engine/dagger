"""Tests for divdag_server.config (TOML loading) + web_dist static serving."""
...
from divdag_server.config import DEFAULT_HOST, DEFAULT_PORT, DivDagConfig, load_config
...
class TestLoadConfig:
    def test_minimal_config_defaults_from_workspace(self, tmp_path: Path) -> None:
        cfg_path = _write(tmp_path / "divdag.toml", 'workspace = "../my-book"\n')
        cfg = load_config(cfg_path)

        assert cfg.workspace == (tmp_path / ".." / "my-book").resolve()
        assert cfg.host == DEFAULT_HOST
        assert cfg.port == DEFAULT_PORT
        assert cfg.resolved_db_url() == sqlite_url(cfg.workspace / "divdag.db")
        assert cfg.resolved_data_dir() == cfg.workspace / ".divdag"
    ...
    def test_missing_workspace_is_a_config_error(self, tmp_path: Path) -> None:
        cfg_path = _write(tmp_path / "divdag.toml", "port = 8000\n")
    ...
    def test_env_fallback_for_db_and_data(
        ...
        monkeypatch.setenv("DIVDAG_DB_URL", "sqlite:///env.db")
        monkeypatch.setenv("DIVDAG_DATA_DIR", str(tmp_path / "env_state"))
        cfg = DivDagConfig(workspace=tmp_path)
...
class TestWebDistServing:
    ...
    def web_dist(self, tmp_path: Path) -> Path:
        d = tmp_path / "dist"
        d.mkdir()
        (d / "index.html").write_text("<html>divdag-spa</html>", encoding="utf-8")
