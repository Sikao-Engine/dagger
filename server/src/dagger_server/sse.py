...
from .events import EventBus, DaggerEvent
...
def _encode(ev: DaggerEvent) -> dict[str, Any]:
    """Encode a DaggerEvent as a ServerSentEventMessage dict (data + id + event)."""
    return {
...
def _safe_put(queue: asyncio.Queue[DaggerEvent | None], ev: DaggerEvent) -> None:
...
class SSEBroker:
    ...
    async def stream(
        ...
        *,
        ...
        queue: asyncio.Queue[DaggerEvent | None] = asyncio.Queue(maxsize=_QUEUE_MAX)

        def _cb(ev: DaggerEvent) -> None:
            try:
