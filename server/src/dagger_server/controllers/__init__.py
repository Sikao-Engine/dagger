...
class StateController(Controller):
    ...
    async def get_item_artifacts(
        ...
        from dagger_kernel.state import group_refs_by_slot
        from dagger_kernel.state.artifact_spec import StateStoreArtifactStore
        ...
        for r in adapter.item_artifacts(run_id, item_id):
            if r.key.slot == slot:
                ...
                return Response(
                    ...
                    headers={"X-Dagger-Slot": slot, "X-Dagger-Sha256": r.sha256},
                )
        raise NotFoundException(
