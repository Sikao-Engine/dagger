...
class StateController(Controller):
    ...
    async def get_item_artifacts(
        ...
        from loom_kernel.state import group_refs_by_slot
        from loom_kernel.state.artifact_spec import StateStoreArtifactStore
        ...
        for r in adapter.item_artifacts(run_id, item_id):
            if r.key.slot == slot:
                ...
                return Response(
                    ...
                    headers={"X-Loom-Slot": slot, "X-Loom-Sha256": r.sha256},
                )
        raise NotFoundException(
