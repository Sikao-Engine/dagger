...
Run it against a workspace described by a TOML config:
    ...
    # or: python -m loom_server -c path/to/loom.toml

Without `-c`, behavior is unchanged: env vars LOOM_DB_URL / LOOM_DATA_DIR /
LOOM_HOST / LOOM_PORT with `./loom.db` + `./.loom` defaults.
"""
...
def _serve(config: LoomConfig, *, host: str, port: int) -> None:
    ...
    print(f"loom workspace: {config.workspace}")
    print(f"loom data dir:  {config.resolved_data_dir()}")
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        prog="loom-server",
        description="Run the Loom server against a workspace (see config.example.toml).",
    )
    parser.add_argument(
        "-c",
        "--config",
        type=Path,
        default=None,
        help="TOML config file. Without it, env vars / ./loom.db + ./.loom defaults apply.",
    )
    ...
    if args.config is not None:
        ...
        config = LoomConfig(workspace=Path.cwd())
