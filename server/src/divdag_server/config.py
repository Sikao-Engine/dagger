"""TOML config loading for the DivDag server.

A config file decouples a DivDag deployment from the repo checkout: the
...
Config shape (see config.example.toml at the repo root):
    ...
    port = 8000                    # optional
    db_url = "sqlite:///..."       # optional; default <workspace>/divdag.db
    data_dir = "state"             # optional; default <workspace>/.divdag
...
class DivDagConfig:
    ...
    def resolved_db_url(self) -> str:
        """Explicit `db_url`, else `DIVDAG_DB_URL` env, else sqlite in the workspace."""
        if self.db_url is not None:
            return self.db_url
        env = os.environ.get("DIVDAG_DB_URL")
        return env if env else sqlite_url(self.workspace / "divdag.db")

    def resolved_data_dir(self) -> Path:
        """Explicit `data_dir`, else `DIVDAG_DATA_DIR` env, else `<workspace>/.divdag`."""
        if self.data_dir is not None:
            return self.data_dir
        env = os.environ.get("DIVDAG_DATA_DIR")
        return Path(env) if env else self.workspace / ".divdag"
...
def load_config(path: str | Path) -> DivDagConfig:
    """Load a TOML config file into a DivDagConfig.
    ...
    return DivDagConfig(
        ...
        host=str(raw.get("host", os.environ.get("DIVDAG_HOST", DEFAULT_HOST))),
        port=int(raw.get("port", os.environ.get("DIVDAG_PORT", DEFAULT_PORT))),
...
__all__ = ["DEFAULT_HOST", "DEFAULT_PORT", "DivDagConfig", "load_config"]
