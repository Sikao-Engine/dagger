...
from .events import EventBus, DivDagEvent
...
def _encode(ev: DivDagEvent) -> dict[str, Any]:
    """Encode a DivDagEvent as a ServerSentEventMessage dict (data + id + event)."""
    return {
...
def _safe_put(queue: asyncio.Queue[DivDagEvent | None], ev: DivDagEvent) -> None:
...
class SSEBroker:
    ...
    async def stream(
        ...
        *,
        ...
        queue: asyncio.Queue[DivDagEvent | None] = asyncio.Queue(maxsize=_QUEUE_MAX)

        def _cb(ev: DivDagEvent) -> None:
            try:
